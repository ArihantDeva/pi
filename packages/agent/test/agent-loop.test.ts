...
import { type BehaviorMeasurement, type BehaviorPolicy, evaluateBehaviorAcceptance } from "../src/behavior-policy.ts";
...
describe("behavior policy contracts", () => {
	it("uses existing prompt, loop, and thinking-level surfaces", () => {
		const policy = {
			promptGuidelines: ["Stop after verified completion."],
			thinkingLevel: "low",
			shouldStopAfterTurn: () => false,
			prepareNextTurn: () => undefined,
		} satisfies BehaviorPolicy;

		expect(policy.promptGuidelines).toEqual(["Stop after verified completion."]);
		expect(policy.thinkingLevel).toBe("low");
	});

	it("accepts a candidate that saves at least 15% without quality or retry regressions", () => {
		const baseline: BehaviorMeasurement[] = [
			{
				taskId: "one",
				inputTokens: 40,
				outputTokens: 60,
		...
		const candidate = baseline.map((measurement) => ({
			...measurement,
			totalTokens: measurement.totalTokens === undefined ? undefined : measurement.totalTokens * 0.8,
		}));

		expect(evaluateBehaviorAcceptance(baseline, candidate)).toMatchObject({
			accepted: true,
			baselineMedianTokens: 200,
			candidateMedianTokens: 160,
			tokenReductionPercent: 20,
			successRateRegressionPoints: 0,
		});
	});

	it("rejects token savings that reduce success or increase retries", () => {
		const baseline: BehaviorMeasurement[] = [
			{
				taskId: "one",
				totalTokens: 100,
		...
		expect(
			evaluateBehaviorAcceptance(
				baseline,
				baseline.map((measurement, index) => ({ ...measurement, totalTokens: 80, success: index !== 3 })),
			),
		).toMatchObject({ accepted: false, reason: "success-rate-regression" });
		expect(
			evaluateBehaviorAcceptance(
				baseline,
				baseline.map((measurement) => ({ ...measurement, totalTokens: 80, retries: 1 })),
			),
