...
describe("AgentActivityTracker", () => {
	test("starts working with no tokens", () => {
		const tracker = new AgentActivityTracker();
		expect(tracker.getStatus()).toEqual({ activity: "working", direction: "up", tokens: 0 });
	});
	...
	test("accumulates tokens across turns and flips direction while executing tools", () => {
		...
		expect(tracker.getStatus()).toEqual({ activity: "working", direction: "up", tokens: 500 });
		...
		expect(tracker.getStatus().activity).toBe("working");
