...
`xhigh`, `max`, and `ultra` are model-specific, opt-in levels. Use `getSupportedThinkingLevels(model)` to determine whether a concrete model exposes them. Codex GPT-5.6 Sol and Terra expose all three; Codex Luna exposes `xhigh` and `max`. Ultra sends the provider's native effort and enables Codex automatic task delegation.
